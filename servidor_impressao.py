...
def carregar_crm():
    ...
    try:
        ...
        with urllib.request.urlopen(CRM_URL, context=ctx, timeout=15) as f:
            crm_html = f.read().decode("utf-8", errors="replace")
        with open(CACHE_FILE, "w", encoding="utf-8") as f:
            f.write(crm_html)
        print("[CRM] HTML atualizado com sucesso.")
    except Exception as e:
        print(f"[CRM] Sem conexao, usando cache: {e}")
        if os.path.exists(CACHE_FILE):
            with open(CACHE_FILE, "r", encoding="utf-8") as f:
                crm_html = f.read()
        else:
            crm_html = "<h1>CRM indisponivel. Verifique a conexao e reinicie.</h1>"

PAGINA_CARREGANDO = """<!doctype html>
<html><head><meta charset="utf-8">
<meta http-equiv="refresh" content="2">
<style>
  body{margin:0;background:#0f1117;color:#c8d0e0;font-family:sans-serif;
       display:flex;align-items:center;justify-content:center;height:100vh;flex-direction:column;gap:16px}
  .dot{width:10px;height:10px;border-radius:50%;background:#7eb8f7;display:inline-block;margin:0 4px;
...
<body>
  ...
  <div style="font-size:18px;font-weight:600">Carregando MagaPet CRM...</div>
  <div><span class="dot"></span><span class="dot"></span><span class="dot"></span></div>
  <div style="font-size:12px;opacity:.5">Esta pagina atualiza automaticamente</div>
</body></html>"""
...
def abrir_chrome():
    ...
    for p in caminhos:
        if os.path.exists(p):
            subprocess.Popen([p, f"http://{IP_LOCAL}:5000/"])
            return

@app.route("/")
def serve_crm():
    if not crm_html:
        return Response(PAGINA_CARREGANDO, content_type="text/html; charset=utf-8")
    return Response(crm_html, content_type="text/html; charset=utf-8")
